...
async function fetchIPLocation() {
  const res = await fetch('https://ip-api.com/json/?lang=pt-BR&fields=status,message,city,regionName,country,countryCode,lat,lon');
  if (!res.ok) throw new Error('Falha ao obter localização pelo IP');
  const data = await res.json();
  if (data.status !== 'success') throw new Error(data.message || 'Localização não encontrada');
...
function renderHourly(weather) {
  ...
  for (let i = 0; i < times.length && count < 24; i++) {
    ...
    el.innerHTML = `
      ...
      <span class="h-temp">${Math.round(weather.hourly.temperature_2m[i])}°</span>
      <span class="h-rain">💧 ${prob || 0}%</span>
    `;
...
function renderRainChart(weather) {
  ...
  daily.time.forEach((date, i) => {
    ...
    const dayLabel = i === 0 ? 'Hoje' : formatDate(date, { weekday: 'short' });

    const row = document.createElement('div');
    row.className = 'rain-bar-row';
    row.innerHTML = `
      <span class="rain-bar-label">${dayLabel} (${prob}%)</span>
...
function renderDaily(weather) {
  ...
  daily.time.forEach((date, i) => {
    ...
    el.innerHTML = `
      ...
      <span class="day-desc">${wmo.desc}${prob > 20 ? ` · 💧${prob}%` : ''}</span>
      <div class="day-temps">
        <span class="day-max">${Math.round(daily.temperature_2m_max[i])}°C</span>
        <span class="day-min">${Math.round(daily.temperature_2m_min[i])}°C</span>
      </div>
    `;
    container.appendChild(el);
  });
...
function renderWind(weather) {
  ...
  daily.time.forEach((date, i) => {
    ...
    row.innerHTML = `
      ...
      <span class="wind-arrow" style="transform: rotate(${dir}deg)" title="${degToCompass(dir)}">↑</span>
...
function renderAlerts(alerts) {
  ...
  alerts.forEach(a => {
    const el = document.createElement('div');
    el.className = `alert-card${a.type === 'danger' ? ' danger' : ''}`;
    el.innerHTML = `
...
// ===== UI State =====
function showLoading(msg = 'Carregando previsão...') {
...
async function loadWeather(location) {
  ...
  try {
    ...
    const alerts = detectAlerts(weather.daily);
...
// ===== Event Listeners & Init (browser only) =====
